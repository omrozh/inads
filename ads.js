...
function createAds(element, index){
  ...
  .then(blob=>{
    ...
    element.insertAdjacentHTML("afterend", '<br><a href="http://inads.herokuapp.com" style="text-decoration: none; margin-left: ' + element.getAttribute("margin-left") + '; margin-right: ' + element.getAttribute("margin-right") + '; color:yellow; position: relative;' + element.getAttribute("height") + '; float: ' + window.getComputedStyle(element).float + '"><small style="margin-left: 12.5%">Ads by <span style="color: black;">InAds</small></span></a>')
